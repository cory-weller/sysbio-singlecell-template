...
adata_subset = adata[adata.obs_names.isin(subset_barcodes)].copy()

# adata.obs['library'] = 'VR001'        # Can derive library from sublibrary at any time
adata.obs['sublibrary'] = adata.obs['library'] + '.' + '0'
# adata.obs['donor'] = snakemake.params.donor   # Can merge in donor from metadata at any time

# prepend barocde name with sublibrary
adata.obs_names = adata.obs['sublibrary'].astype(str) + '_' +  adata.obs_names
