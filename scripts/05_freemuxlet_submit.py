...
# Load library-sample composition file
library_metadata_file = find_file(config.metadata.sample_libraries)
df = pd.read_csv(library_metadata_file, sep=',')
df = df[['specimenID','libraryBatch']]
library_sample_counts = df.groupby(['libraryBatch']).size()
library_sample_counts[libraryID] # to get # of samples in batch
...
if config.step.rerun_debug is None:
    for libraryID in df['libraryID']:
        output = Path(data_dir / 'DEMUX' / libraryID /'freemuxlet.clust1.vcf.gz') # TODO
        require_path(path=output.parent, kind='dir', label='demux output directory', create=True)
        if output.exists():
            ids_finished.append(libraryID)
        elif not output.exists() or config.step.clobber is True:
            # Ensure inputs exist
            bamfile = require_path(path=data_dir / 'CELLRANGER' / libraryID /'possorted_genome_bam.bam', label='cellranger bam', kind='file', create=False)
